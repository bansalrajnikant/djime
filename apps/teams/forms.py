...
class TeamForm(forms.ModelForm):
    ...
    def clean_slug(self):
        ...
        if Team.objects.filter(slug__iexact=self.cleaned_data["slug"]).count() > 0:
            raise forms.ValidationError(_("A team already exists with that slug."))
        return self.cleaned_data["slug"].lower()

    def clean_name(self):
        if Team.objects.filter(name__iexact=self.cleaned_data["name"]).count() > 0:
