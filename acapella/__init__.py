...
class Slip(MetaObject):
    def has_permission(self, user_input):
        ...
        perm = PermissionSlip.objects.filter(model=self, user=user)
    ...
    def check_permission(self, user_input, permission):
        ...
        # returning False to make sure new users dont have access to everything.
        if not perm:
            return False
    ...
    def add_permission(self, user_input, permission):
        ...
        perm = PermissionSlip.objects.filter(model=self, user=user)
        if not perm:
            new_perm = PermissionSlip(model=self, user=user, permission=perm_int)
            new_perm.save()
    ...
    def del_permission(self, user_input, permission):
        ...
        if not perm:
            ...
            new_perm = PermissionSlip(model=self, user=user, permission=0)
            new_perm.save()
...
class Project(MetaObject):
    def has_permission(self, user_input):
        ...
        if user.is_superuser:
            return "User %s is superuser and has all access" % user
        perm = PermissionProject.objects.filter(model=self, user=user)
        # check to see if there is any info for the given user and object
        if not perm:
            return "No permissions has been entered for user %(user)s and object %(object)s" % {'user': user, 'object': self}
        else:
    ...
    def add_permission(self, user_input, permission):
        ...
        perm = PermissionProject.objects.filter(model=self, user=user)
        if not perm:
            new_perm = PermissionProject(model=self, user=user, permission=perm_int)
            new_perm.save()
            return 'New permission %(permission)s for user %(user)s on object %(model)s has been created' % {'permission': permission,
                                                                                                            'user': user,
                                                                                                            'model': self}
    ...
    def del_permission(self, user_input, permission):
        ...
        if not perm:
            # if no permission has been created for the user on the object, we
            # create a new permission with no access perm_int = 0
            new_perm = PermissionProject(model=self, user=user, permission=0)
            new_perm.save()
            return 'New permission for user %(user)s on object %(model)s with no access has been created' % {'user': user,
                                                                                                            'model': self}
        else:
...
class Client(MetaObject):
    def has_permission(self, user_input):
        ...
        if user.is_superuser:
            return "User %s is superuser and has all access" % user
        perm = PermissionClient.objects.filter(model=self, user=user)
        # check to see if there is any info for the given user and object
        if not perm:
            return "No permissions has been entered for user %(user)s and object %(object)s" % {'user': user, 'object': self}
        else:
    ...
    def add_permission(self, user_input, permission):
        ...
        perm = PermissionClient.objects.filter(model=self, user=user)
        if not perm:
            new_perm = PermissionClient(model=self, user=user, permission=perm_int)
            new_perm.save()
            return 'New permission %(permission)s for user %(user)s on object %(model)s has been created' % {'permission': permission,
                                                                                                            'user': user,
                                                                                                            'model': self}
    ...
    def del_permission(self, user_input, permission):
        ...
        if not perm:
            # if no permission has been created for the user on the object, we
            # create a new permission with no access perm_int = 0
            new_perm = PermissionClient(model=self, user=user, permission=0)
            new_perm.save()
            return 'New permission for user %(user)s on object %(model)s with no access has been created' % {'user': user,
                                                                                                            'model': self}
        else:
