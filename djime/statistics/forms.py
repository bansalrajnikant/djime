...
class DateSelectionForm(forms.Form):
    ...
    def clean(self):
        ...
        # if a data has been entered, we will validate that the imput by the
        # user is acceptable.
        if date_data:
            # first check that the user has entered 2 values seperated by space something space.
            date_list = date_data.split()
            if len(date_list) != 3:
                raise forms.ValidationError(trans("You have to enter 2 dates, in the date field, seperated by SPACE and a DASH (-) and a SPACE. Hint, use the date picker provided"))
            else:
                # start and end dates are set from the date list.
                start_date = date_list[0]
                end_date = date_list[-1]
                # use regular expression to check if the user has entered the
                # date in the format 'yyyy-mm-dd'
                if not re.match("[0-9]{4}[-]{1}[0-9]{2}[-]{1}[0-9]{2}$", start_date):
                    raise forms.ValidationError(trans("Start date has invalid format, must be 'yyyy-mm-dd'"))
                ...
                try:
                    ...
                    raise forms.ValidationError(trans("Start date does not exist"))
                try:
                    e_date = datetime.date(int(end[0]),int(end[1]),int(end[2]))
                except ValueError:
                    raise forms.ValidationError(trans("End date does not exist"))
                ...
                if e_date - (s_date) > datetime.timedelta(days=60):
                    raise forms.ValidationError(trans("Difference between end and start date must be lower than 60 days"))
