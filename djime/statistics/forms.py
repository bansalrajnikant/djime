...
class DateSelectionForm(forms.Form):
    ...
    def clean(self):
        ...
        if date_data:
            ...
            if len(date_list) != 3:
                raise forms.ValidationError(_("You have to enter 2 dates, in the date field, seperated by SPACE. Hint, use the date picker provided"))
            else:
                ...
                if not re.match("[0-9]{4}[-]{1}[0-9]{2}[-]{1}[0-9]{2}$", start_date) or not re.match("[0-9]{4}[-]{1}[0-9]{2}[-]{1}[0-9]{2}$", end_date):
                    if start_date == end_date:
                        raise forms.ValidationError(_("You have to choose 2 different days"))
